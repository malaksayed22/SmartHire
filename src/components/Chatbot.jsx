...
function getBotReply(text) {
  ...
  if (/job|position|role|opening|hiring|vacanc/.test(t)) {
    const active = JOBS.filter(j => j.status === 'active');
    const list = active.map(j => `• **${j.title}** — ${j.department} · ${j.location}`).join('\n');
    return {
      text: `We currently have ${active.length} open positions:\n\n${list}\n\nHead to the Jobs page to read full descriptions and apply!`,
  ...
  if (/salary|pay|compens|money|benefit|package/.test(t)) {
    const salaries = JOBS.filter(j => j.status === 'active').map(j => `• ${j.title}: ${j.salary}`).join('\n');
    return {
  ...
  if (/remote|location|where|cairo|egypt|hybrid|on.?site/.test(t)) {
    const locs = JOBS.filter(j => j.status === 'active').map(j => `• ${j.title}: ${j.location}`).join('\n');
    return {
  ...
  if (/department|design|engineer|hr|research|ml|frontend|backend/.test(t)) {
    const depts = [...new Set(JOBS.filter(j => j.status === 'active').map(j => j.department))];
    return {
      text: `We're currently hiring across these departments:\n\n${depts.map(d => `• ${d}`).join('\n')}\n\nCheck the Jobs page and filter by department to find roles that suit you.`,
      quick: ['What jobs are open?', 'How do I apply?'],
    };
  }
...
function TypingDots() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '12px 16px', background: 'var(--s3)', borderRadius: '18px 18px 18px 4px', width: 'fit-content' }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{
          width: 7, height: 7, borderRadius: '50%', background: 'var(--m2)',
...
export default function Chatbot() {
  ...
  const isHR = location.pathname.startsWith('/hr/dashboard') ||
  ...
  return (
    <>
      ...
      <button
        ...
        onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.08)'; e.currentTarget.style.animation = 'none'; }}
        onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; if (!open) e.currentTarget.style.animation = 'chatPulse 2.5s infinite'; }}
      >
        {open ? (
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M3 3l12 12M15 3L3 15" stroke="#fff" strokeWidth="2.2" strokeLinecap="round"/>
          </svg>
        ) : (
      ...
      {open && (
        <div style={{
          ...
          zIndex: 9998,
          overflow: 'hidden',
          boxShadow: '0 24px 64px rgba(0,0,0,0.6), 0 0 0 1px rgba(91,142,248,0.08)',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
                <path d="M12 2a5 5 0 015 5v1a5 5 0 01-10 0V7a5 5 0 015-5z" stroke="#fff" strokeWidth="1.8" strokeLinecap="round"/>
                <path d="M3 20c0-4 4-7 9-7s9 3 9 7" stroke="#fff" strokeWidth="1.8" strokeLinecap="round"/>
              </svg>
            </div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 1 }}>
                ...
                <span style={{ fontSize: 11.5, color: 'var(--teal)', fontWeight: 500 }}>Online</span>
              </div>
            </div>
            <button
              onClick={() => setOpen(false)}
              style={{ background: 'none', border: 'none', color: 'var(--m2)', cursor: 'pointer', padding: 4, borderRadius: 6, lineHeight: 0 }}
          ...
          <div style={{
            ...
            {messages.map((msg, i) => (
              <div key={i}>
                ...
                {/* Quick replies */}
                {msg.from === 'bot' && msg.quick && i === messages.length - 1 && !typing && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8, paddingLeft: 2 }}>
                    {msg.quick.map((q, qi) => (
                      <button key={qi} className="chat-msg-btn" onClick={() => sendMessage(q)}>
                        {q}
                      </button>
                    ))}
                  </div>
          ...
          <div style={{
            ...
            <input
              ...
              onFocus={e => e.target.style.borderColor = 'rgba(91,142,248,0.5)'}
